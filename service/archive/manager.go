...
func metadataFor(manifest Manifest, objectType string, object ObjectInfo) map[string]string {
	return map[string]string{
		...
		"trace_id_present": fmt.Sprintf("%t", manifest.TraceIDPresent),
...
func patchArchive(c *gin.Context, manifest Manifest) {
	info := map[string]interface{}{
		...
		"request_id":       manifest.RequestID,
		"trace_id":         manifest.TraceID,
		"trace_id_present": manifest.TraceIDPresent,
...
func logArchiveEvent(c *gin.Context, manifest Manifest, status string, reason string, detail string) {
	msg := fmt.Sprintf("archive %s: request_id=%s trace_id=%s backend=%s reason=%s detail=%s", status, manifest.RequestID, manifest.TraceID, manifest.Backend, reason, detail)
