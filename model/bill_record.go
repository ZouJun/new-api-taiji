...
type BillRecord struct {
	...
	CacheTokens                   int     `json:"cache_tokens" gorm:"default:0;comment:缓存读取 Token 数量，用于缓存命中费用计算"`
	...
	Number                        int     `json:"number" gorm:"default:1;comment:成功且产生计费的调用数量，按次计费时用于费用计算"`
...
func buildBillPricingContext(other map[string]interface{}) billPricingContext {
	return billPricingContext{
		...
		CacheCreationRatio:   mapFloatValue(other, "cache_creation_ratio", "cache_creation_ratio_5m"),
...
func recordBillFromConsumeLog(userId int, username string, createdAt int64, requestId string, params RecordConsumeLogParams, clientRequestId string) {
	...
	record := &BillRecord{
		...
		CacheCreationTokens:   mapIntValue(other, "cache_creation_tokens", "cache_write_tokens"),
		CacheCreation1hTokens: mapIntValue(other, "cache_creation_tokens_1h"),
...
func hasTokenUsage(record *BillRecord) bool {
	return record.PromptTokens > 0 || record.CompletionTokens > 0 || record.CacheTokens > 0 || record.CacheCreationTokens > 0 || record.CacheCreation1hTokens > 0 || record.AudioInputTokens > 0 || record.ImageInputTokens > 0
}
...
func computeBillPrices(record *BillRecord, pricingContext billPricingContext, discount float64) billPrices {
	...
	if hasTokenUsage(record) {
		consumeCost += float64(record.PromptTokens) / 1_000_000 * baseTokenPrice
		consumeCost += float64(record.CacheTokens) / 1_000_000 * cachePrice
	...
	if !hasTokenUsage(record) && record.MediaDurationMs == 0 && record.Number > 0 {
...
func GetBillAliDayList(startAt, endAt int64, modelPrefix string) ([]*BillAliDayListItem, error) {
	...
	for _, item := range aggregated {
		item.ActualInputToken = item.InputToken - item.CachedInputToken
		if item.ActualInputToken < 0 {
			item.ActualInputToken = 0
		}
